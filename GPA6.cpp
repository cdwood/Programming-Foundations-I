...
void getCourse(string& name, string& time, string& number, char& grade, int& hours, Course& courses, int n)//Takes in the total number of courses, and the reference to a specific point in the arrays for times, numbers, grades, hours, and courses, and stores user input in these variables.
{
	bool valid = true;
	cin.clear();
	cin.ignore(256, '\n');
	...
	courses.set(name, time, number, grade, hours);//sets the value of this specific course object to the values being stored in the parallel arrays.
}

bool removeCourse(string name[], string time[], string number[], char grade[], int hours[], Course courses[], int& n)//takes in the array for name, time, number, grade, hours, and courses, as well as the value for the number courses and removes a course specified by the user.
{
...
bool reading(const char filename[], string name[], string time[], string number[], char grade[], int hours[], Course courses[], int& n, int capacity)//takes in a filename to be read, the arrays for name, time, number, grade, hours, and courses, as well as the number of objects in these arrays and the capacities of these arrays.
{
	fstream infile;
	...
	for(int i = 0; i<n; i++)//takes in teh value and stores it in the arrays for each piece of information in the file.
	{
		...
		courses[i].set(name[i], time[i], number[i], grade[i], hours[i]);//stores the information that is stroed in the parallel arrays at this instance in the very same instance of course object in the course array.
	}
	infile.close();
	return true;
}

bool writing(const char filename[], const string name[], const string time[], const string number[], const char grade[], const int hours[], int n)//Takes in a filename as well as the arrays for name, time, number, grade, and hours, along with the variable that stores the number of items stored in these arrays.
{
	ofstream outfile;
...
bool coursesChecking(const char filename[], int n, Course courses[])//takes in a filename and the array of courses and outputs all of the information in the array to the file
{
	...
	ofstream outfile;
	outfile.open(filename);
	if(outfile.fail())//verifies that the file can be opened
	{
		cout << "failed to open file named: " << filename << endl;
		return false;
	}
	outfile << n << endl;
	for(int i = 0; i < n; i++)//gets the information for each course object instance in the array and outputs that to the file
	{
...
int main()
{
	...
	while(valid)//asks if the user wants to use an input file or manual input and acts accordingly
	{
		...
		if(menuOp == 'y')
		{
			...
			while(valid)//asks if the user wants to use the default file or specify a file and acts accordingly
			{
				...
				if(menuOp == 'y')
				{
					if(!(reading(filename.c_str(), names, times, numbers, grades, hours, courses, courseAmt, 10)))
					{
		...
		else if(menuOp == 'n')
		{
			...
			{
				getCourse(names[i], times[i], numbers[i], grades[i], hours[i], courses[i], i);
			}
			valid = false;
		}
		else
	...
	while(valid)//loops the menu until menu option Q is chosen.
	{
		...
		else if((menuOp == 'E')||(menuOp == 'e'))//If option E is chosen it calls the getCourse function and adds one to the total number of courses.
		{
			if(courseAmt < 10)
			{
				getCourse(names[courseAmt], times[courseAmt], numbers[courseAmt], grades[courseAmt], hours[courseAmt], courses[courseAmt], courseAmt);
		...
		{
			if(removeCourse(names, times, numbers, grades, hours, courses, courseAmt))
			{
				cout << "Please review the updated courses below\n";
				print(courseAmt, names, times, numbers, grades, hours);
			}
		...
		{
			if(!(writing("courses.txt", names, times, numbers, grades, hours, courseAmt)))//calls writing and outputs all information in the arrays to "courses.txt"
			{
				cout << "Error in writing to the output file. Terminating..." << endl;
				return 1;
			}
			if(!(coursesChecking("checking.txt", courseAmt, courses)))//calls coursesChecking and outputs all information in the array of Courses to "checking.txt"
			{
				cout << "Could not verify Course class functionality." << endl;
				return 1;
			}
			valid = false;
